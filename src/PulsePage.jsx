  ...
  return (
    <div onMouseMove={handleMouseMove} style={{ background: "#09090B", color: "#FAFAFA", fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", minHeight: "100vh", position: "relative" }}>
      ...
      <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: isMobile ? "12px 20px" : "20px 48px", position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(9,9,11,0.92)", backdropFilter: "blur(24px)", borderBottom: `1px solid ${VG(0.08)}` }}>
        <img src="/logo-nav.png" alt="NERVÜR" onClick={() => navigate("/")} style={{ height: isMobile ? "34px" : "42px", width: "auto", objectFit: "contain", cursor: "pointer", filter: "invert(1) brightness(1.15)" }} />
      ...
      <main style={{ padding: isMobile ? "100px 16px 60px" : "140px 48px 80px", maxWidth: "1100px", margin: "0 auto" }}>
        ...
        <section aria-label="Statistiques de risque" style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: "16px", marginBottom: "32px", animation: "fadeInUp 0.6s ease 0.1s both" }}>
          {[
            ...
            <div key={i} style={{ padding: "20px", border: `1px solid ${VG(0.1)}`, background: "rgba(24,24,27,0.5)" }}>
              <div style={{ fontSize: "9px", color: "#52525B", letterSpacing: "2px", marginBottom: "8px" }}>{s.label}</div>
              <span style={{ fontSize: "28px", fontWeight: 800, color: s.color }}>{s.value}</span>
              {s.suffix && <span style={{ fontSize: "14px", color: "#52525B", marginLeft: "4px" }}>{s.suffix}</span>}
            </div>
          ))}
        </section>
              ...
              <div style={{ marginTop: "20px" }}>
                <div style={{ display: "flex", justifyContent: "center", gap: "24px", fontSize: "10px", letterSpacing: "1px" }}>
                  {["Collecte données", "Analyse comportement", "Calcul prédictif", "Génération alertes"].map((step, i) => (
                    <span key={i} style={{ color: analysisStep > i ? "#d946ef" : "#52525B", transition: "color 0.3s", animation: analysisStep === i + 1 ? "scanPulse 0.8s ease infinite" : "none" }}>
        ...
        {scanComplete && (
          <section aria-label="Tableau des clients et risques de churn" style={{ animation: "fadeInUp 0.5s ease both" }}>
            <div style={{ border: `1px solid ${VG(0.1)}`, background: "rgba(24,24,27,0.4)", overflow: "hidden", marginBottom: "32px" }}>
              {/* Header */}
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 80px" : "2fr 1fr 1fr 1fr 120px", padding: "12px 20px", borderBottom: `1px solid ${VG(0.08)}`, background: "rgba(24,24,27,0.6)" }}>
                {(isMobile ? ["Client", "Risque"] : ["Client", "Revenu", "Utilisation", "Satisfaction", "Risque Churn"]).map((h, i) => (
                  <span key={i} style={{ fontSize: "9px", letterSpacing: "2px", color: "#52525B", textTransform: "uppercase", textAlign: i > 0 ? "center" : "left" }}>{h}</span>
                ))}
              </div>
              {/* Rows */}
              {CLIENTS.map((c, i) => (
                <div key={i} onClick={() => setSelectedClient(selectedClient === i ? null : i)}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "10px", color: "#52525B" }}>{c.sector}</div>
                  </div>
                  ...
                  {!isMobile && (
                    <div style={{ textAlign: "center", alignSelf: "center" }}>
                      ...
                      <span aria-hidden="true" style={{ fontSize: "9px", color: "#52525B", marginTop: "2px", display: "block" }}>{c.usage}%</span>
                    </div>
                  )}
            ...
            {selectedClient !== null && (
              <div style={{ animation: "fadeInUp 0.4s ease both", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "20px", marginBottom: "32px" }}>
                <div style={{ padding: "24px", border: `1px solid ${VG(0.1)}`, background: "rgba(24,24,27,0.4)" }}>
        ...
        <section aria-label="Appel à l'action" style={{ marginTop: "60px", textAlign: "center", padding: isMobile ? "32px 20px" : "48px", border: `1px solid ${VG(0.1)}`, background: "rgba(24,24,27,0.3)", animation: "fadeInUp 0.6s ease 0.4s both" }}>
          ...
          <p style={{ fontSize: "14px", color: "#71717A", marginBottom: "28px", maxWidth: "450px", margin: "0 auto 28px", lineHeight: 1.7 }}>PULSE s'intègre à votre CRM et commence à prédire dès le premier jour.</p>
          <button onClick={() => navigate('/contact?outil=pulse')} style={{ padding: "14px 36px", background: V, color: "#09090B", border: "none", fontWeight: 800, fontSize: "12px", letterSpacing: "1.5px", textTransform: "uppercase", cursor: "pointer", transition: "all 0.3s", fontFamily: "inherit" }}
