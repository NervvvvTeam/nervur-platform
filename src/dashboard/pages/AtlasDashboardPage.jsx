...
function ChangeBadge({ change }) {
  if (!change || change === 0) return <span style={{ color: "#6B7C93", fontSize: "12px" }}>--</span>;
  const up = change > 0;
  return (
    <span style={{
...
function CompetitionBadge({ level }) {
  if (!level) return <span style={{ color: "#6B7C93", fontSize: "12px" }}>--</span>;
  const colors = {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
          <h3 style={{ color: "#0A2540", fontSize: "16px", fontWeight: 600, margin: 0 }}>
            Suggestions IA
          </h3>
            ...
            {suggestions.map((s, i) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: "#0A2540", fontSize: "13px", fontWeight: 500 }}>{s.keyword}</div>
                  <div style={{ color: "#6B7C93", fontSize: "11px", marginTop: "2px" }}>{s.reason}</div>
                </div>
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ color: "#6B7C93", fontSize: "11px", marginTop: "8px" }}>Score SEO global</div>
            </div>

            {/* Summary */}
            <div style={{ background: "#16171f", borderRadius: "10px", padding: "16px", border: "1px solid #2a2d3a" }}>
              <div style={{ color: "#425466", fontSize: "13px", lineHeight: "1.6" }}>{report.summary}</div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(100px, 1fr))", gap: "10px" }}>
                {[
                  ...
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: "16px", fontWeight: 700, color: "#0A2540" }}>{s.value}</div>
                    <div style={{ fontSize: "10px", color: "#6B7C93", marginTop: "4px" }}>{s.label}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {report.forecast && (
              <div style={{ background: "rgba(245,158,11,0.08)", borderRadius: "10px", padding: "16px", border: "1px solid rgba(245,158,11,0.15)" }}>
                ...
                <div style={{ color: "#425466", fontSize: "13px", lineHeight: "1.5" }}>{report.forecast}</div>
              </div>
            )}

            {report.generatedAt && (
              <div style={{ textAlign: "right", fontSize: "10px", color: "#6B7C93" }}>
...
function ReportSection({ title, items, color, icon }) {
  ...
  return (
    <div>
      <div style={{ fontSize: "12px", fontWeight: 600, color: "#0A2540", marginBottom: "8px" }}>{title}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
        {items.map((item, i) => (
          <div key={i} style={{
            ...
            <span style={{ color: "#425466", fontSize: "12px", lineHeight: "1.4" }}>{item}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "16px", marginBottom: "28px" }}>
        {[
          ...
          { label: "Mots-cles Top 10", value: keywordsInTop10, suffix: "", color: keywordsInTop10 > 0 ? "#22c55e" : "#6B7C93" },
        ].map((stat, i) => (
          <div key={i} style={{
            background: "#FFFFFF", borderRadius: "12px", padding: "20px",
            border: "1px solid #2a2d3a",
          }}>
            <div style={{ fontSize: "11px", color: "#6B7C93", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>{stat.label}</div>
            <div style={{ fontSize: "24px", fontWeight: 700, color: stat.color || "#0A2540" }}>{stat.value}{stat.suffix}</div>
          </div>
        ))}
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} style={{
          ...
          <h3 style={{ color: "#0A2540", fontSize: "15px", fontWeight: 600, margin: "0 0 16px" }}>Nouveau projet SEO</h3>
          {error && (
          ...
          <div style={{ marginBottom: "14px" }}>
            <label style={{ display: "block", fontSize: "12px", color: "#6B7C93", marginBottom: "6px" }}>Domaine</label>
            <input
          ...
          <div style={{ marginBottom: "16px" }}>
            <label style={{ display: "block", fontSize: "12px", color: "#6B7C93", marginBottom: "6px" }}>Mots-cles (un par ligne)</label>
            <textarea
          ...
          <button type="submit" disabled={creating} style={{
            ...
            background: creating ? "#6B7C93" : "linear-gradient(135deg, #f59e0b, #fbbf24)",
            color: creating ? "#6B7C93" : "#000", fontSize: "13px", fontWeight: 600,
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <h3 style={{ color: "#0A2540", fontSize: "16px", fontWeight: 600, margin: "0 0 8px" }}>Aucun projet SEO</h3>
          <p style={{ color: "#6B7C93", fontSize: "13px", margin: 0 }}>Creez votre premier projet pour commencer le suivi de vos positions Google.</p>
        </div>
      ) : (
              ...
              <div key={project._id} style={{
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                    ...
                    <div>
                      <div style={{ color: "#0A2540", fontSize: "14px", fontWeight: 600 }}>{project.domain}</div>
                      <div style={{ color: "#6B7C93", fontSize: "12px", marginTop: "2px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                    {project.averagePosition && (
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: "11px", color: "#6B7C93" }}>Pos. moy.</div>
                        <PositionBadge position={project.averagePosition} />
                      </div>
                    )}
                    {projectTraffic > 0 && (
                      <div style={{ textAlign: "center" }}>
                        <div style={{ fontSize: "11px", color: "#6B7C93" }}>Trafic est.</div>
                ...
                {isExpanded && (
                  <div style={{ borderTop: "1px solid #2a2d3a", padding: "20px 24px" }}>
                    ...
                    <div style={{ display: "flex", gap: "10px", marginBottom: "20px", flexWrap: "wrap" }}>
                      <button
                        ...
                        style={{
                          ...
                          background: checking === project._id ? "#6B7C93" : "linear-gradient(135deg, #f59e0b, #fbbf24)",
                          color: checking === project._id ? "#6B7C93" : "#000",
                    ...
                    {project.rankings && project.rankings.length > 0 ? (
                          ...
                          <tbody>
                            {project.rankings.map((r, i) => (
                              <tr key={i} style={{ borderBottom: "1px solid #2a2d3a20" }}>
                                <td style={{ padding: "10px 12px", color: "#0A2540", fontSize: "13px", fontWeight: 500 }}>
                                  {r.keyword}
                                </td>
                                ...
                                <td style={{ padding: "10px 12px", color: "#425466", fontSize: "12px" }}>
                                ...
                                <td style={{ padding: "10px 12px", color: "#6B7C93", fontSize: "12px", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                  {r.url}
                                </td>
                              </tr>
                            ))}
                      ...
                      <div style={{ textAlign: "center", padding: "24px 0", color: "#6B7C93", fontSize: "13px" }}>
