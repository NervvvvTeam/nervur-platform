    ...
    <div style={{ maxWidth: "1100px" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h1 style={{ fontSize: "22px", fontWeight: 600, color: "#0F172A" }}>Avis clients</h1>
          <span style={{ fontSize: "12px", color: "#334155" }}>{pagination?.total || 0} avis au total</span>
        </div>
        <p style={{ fontSize: "14px", color: "#64748B" }}>Consultez et gérez les avis de vos clients.</p>
      </div>
          ...
          {FILTERS.map(f => (
            <button key={f.key} onClick={() => { setSentiment(f.key); setPage(1); }}
              style={{
                ...
                border: `1px solid ${sentiment === f.key ? `${f.color}45` : "#E2E8F0"}`,
                borderRadius: "6px", color: sentiment === f.key ? f.color : "#64748B",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
        {reviews.length > 0 ? reviews.map(r => (
          ...
          <div style={{ padding: "60px", textAlign: "center", color: "#334155", border: "1px dashed #e5e7eb", borderRadius: "10px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginTop: "24px" }}>
          {Array.from({ length: pagination.pages }, (_, i) => (
            <button key={i} onClick={() => setPage(i + 1)}
              style={{
                width: "36px", height: "36px", borderRadius: "8px", border: `1px solid ${page === i + 1 ? "#6366f150" : "#E2E8F0"}`,
                background: page === i + 1 ? "linear-gradient(135deg, #6366f1, #818CF8)" : "transparent",
                color: page === i + 1 ? "#ffffff" : "#64748B",
