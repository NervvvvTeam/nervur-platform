    ...
    <div className="text-center">
      <div className="relative mx-auto" style={{ width: size, height: size }}>
        <svg width={size} height={size} className="-rotate-90">
          <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke="#E3E8EE" strokeWidth="5" />
        ...
        <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center">
          <span className="font-semibold text-[#0A2540]" style={{ fontSize: size > 70 ? "22px" : "18px" }}>{score}</span>
        </div>
      </div>
...
function CWVItem({ label, data }) {
  ...
  return (
    <div className="px-[18px] py-[14px] bg-white border border-[#E3E8EE] rounded-lg flex justify-between items-center shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
      <div>
        <div className="text-[11px] text-[#9ca3af] mb-1">{label}</div>
        <div className="text-[17px] font-semibold text-[#0A2540]">
          {data.display || `${data.value}${data.unit}`}
        </div>
      </div>
      <div className="w-2.5 h-2.5 rounded-full" style={{ background: colors[data.status] || "#6B7C93" }} />
    </div>
  );
}
...
export default function PhantomDashboardPage() {
  ...
  return (
    <div className="max-w-[1100px]">
      ...
      {/* URL Input */}
      <div className="flex gap-2.5 mb-8 p-5 bg-white border border-[#E3E8EE] rounded-[10px] shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
        <input
          type="text" value={url} onChange={e => setUrl(e.target.value)}
          placeholder="https://exemple.com"
          onKeyDown={e => e.key === "Enter" && !loading && runAudit()}
          className="flex-1 px-4 py-3 bg-white border border-[#E3E8EE] rounded-lg text-[#0A2540] text-sm font-[inherit] outline-none box-border transition-[border-color,box-shadow] duration-200 focus:border-[#8b5cf6] focus:shadow-[0_0_0_3px_rgba(139,92,246,0.15)]"
        />
      ...
      {result && (
        <>
            ...
            {result.auditId && previousAudits.length > 0 && (
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-3 py-2 bg-white border border-[#E3E8EE] rounded-lg text-[#0A2540] text-[13px] font-[inherit] cursor-pointer outline-none min-w-[220px]"
                >
          ...
          {/* Comparison results */}
          {comparingLoading && (
            <div className="p-5 bg-white border border-[#E3E8EE] rounded-[10px] mb-4 text-center text-[#9ca3af] text-sm">
              Comparaison en cours...
            </div>
          )}

          {comparison && !comparingLoading && (
            <div className="p-6 bg-white border border-[#E3E8EE] border-l-[3px] border-l-[#a78bfa] rounded-[10px] mb-4 shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
              ...
              <div className="grid grid-cols-5 gap-3 mb-5">
                {["global", "performance", "accessibility", "seo", "bestPractices"].map(key => {
                  ...
                  return (
                    <div key={key} className="px-2.5 py-[14px] bg-[#161820] border border-[#E3E8EE] rounded-lg text-center">
                      ...
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <span className="text-[15px] font-semibold text-[#0A2540]">{comp.current}</span>
                      </div>
          ...
          {/* Scores */}
          <div className="p-7 bg-white border border-[#E3E8EE] border-l-[3px] border-l-[#8b5cf6] rounded-[10px] mb-4 shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
          ...
          {result.coreWebVitals && (
            <div className="p-6 bg-white border border-[#E3E8EE] border-l-[3px] border-l-[#3b82f6] rounded-[10px] mb-4 shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
          ...
          {/* Issues */}
          <div className="p-6 bg-white border border-[#E3E8EE] rounded-[10px] shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
              ...
              <div className="flex gap-1.5">
                {["all", "critical", "warning", "info"].map(f => (
                  <button key={f} onClick={() => setFilter(f)}
                    ...
                    style={{
                      background: filter === f ? (f === "all" ? "#E3E8EE" : SEVERITY_COLORS[f] + "20") : "transparent",
                      color: filter === f ? (f === "all" ? "#0A2540" : SEVERITY_COLORS[f]) : "#6B7C93",
                    }}>
            ...
            <div className="flex flex-col gap-2">
              {filteredIssues.map((issue, i) => (
                <div key={i} className="px-5 py-4 bg-white border border-[#E3E8EE] rounded-lg shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
                  style={{ borderLeft: `3px solid ${SEVERITY_COLORS[issue.severity] || "#6B7C93"}` }}>
                  <div className="flex items-center gap-2.5 mb-1.5">
                    <span className="text-[11px] font-medium px-2 py-0.5 rounded"
                      style={{
                        background: (SEVERITY_COLORS[issue.severity] || "#6B7C93") + "18",
                        color: SEVERITY_COLORS[issue.severity] || "#6B7C93",
                      }}>
                      {issue.severity === "critical" ? "Critique" : issue.severity === "warning" ? "Attention" : "Info"}
                    </span>
                    <span className="text-[11px]" style={{ color: CATEGORY_COLORS[issue.category] || "#6B7C93" }}>
          ...
          <div className="mt-6 text-center">
            <button onClick={() => { setResult(null); setUrl(""); setError(""); setProgress(0); setComparison(null); setSelectedCompareId(""); setPreviousAudits([]); }}
              className="px-6 py-2.5 bg-transparent border border-[#E3E8EE] rounded-lg text-[#6b7280] text-[13px] cursor-pointer font-[inherit]">
              Nouvel audit
            </button>
          </div>
        </>
